...
namespace OllamaClient.Services
{
    ...
    /// </summary>
    public class SerializeableStorageService : ISerializeableStorageService
    {
        ...
        public SerializeableStorageService(ILogger<SerializeableStorageService> logger, IOptions<Settings> options)
        {
            ...
            {
                Settings.DirectoryOption.AppData => $"{Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData)}\\OllamaClient\\Storage",
            ...
            foreach (var path in Directory.GetFiles(assemblyFolder, "OllamaClient*.dll"))
            {
                assemblies.Add(Assembly.LoadFrom(path));
            }

            // Iterate through all files in _Directory
            foreach (string file in Directory.GetFiles(_Directory))
            {
                ...
                foreach(Assembly assembly in assemblies)
                {
                    ...
                    if (assembly.GetType(fileName) is Type fileType)
                    {
